...
const Avail = () => {
  ...
  const [ques1] = useState(
    "What are the subjects not available on this semester?"
  );
  const [ques2] = useState("What are the subjects not taken yet?");
  const [ques3] = useState("What's your current semester?");
  const [desc, setDesc] = useState("");
  const [inctype] = useState(
    "Subjects that are not available on the current semester not yet taken"
  );
  const [status] = useState("Active");
  const [uid] = useState("ID: #" + getRandomInt(1000000));
